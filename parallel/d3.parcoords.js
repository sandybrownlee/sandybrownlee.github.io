...
pc.commonScale = function(global, type) {
  ...
  // Removing the factors that are unticked in the checkboxes
  var selected_factors = d3.keys(pc.types());

  for(var key in factors_to_include){    
    if(!factors_to_include[key]){     
      var x = selected_factors.indexOf(key);     
      selected_factors.splice(x,1);
    }    
  }

  pc.dimensions(selected_factors);
  return this;
};
